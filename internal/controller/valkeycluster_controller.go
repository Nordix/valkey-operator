...
func (r *ValkeyClusterReconciler) assignSlotsToPendingPrimaries(ctx context.Context, cluster *valkeyiov1alpha1.ValkeyCluster, state *valkey.ClusterState, nodes *valkeyiov1alpha1.ValkeyNodeList) (int, error) {
	...
	for _, node := range primaries {
		...
		log.V(1).Info("assign slots to primary", "node", node.Address, "ranges", nodeRanges, "unassignedSlots", slots, "remainingPrimaries", remainingPrimaries, "numSlotsForNode", numSlotsForNode)
		...
		log.Info("slots assigned successfully",
			"node", node.Address,
			"nodeId", node.Id,
			"slotRanges", valkey.FormatSlotsRanges(nodeRanges))
